...
### 8. TCP порт – 16 битное число. Предположим, 2 находящихся в одной сети хоста устанавливают между собой соединения. Каким будет теоретическое максимальное число соединений, ограниченное только лишь параметрами L4, которое параллельно может установить клиент с одного IP адреса к серверу с одним IP адресом? Сколько соединений сможет обслужить сервер от одного клиента? А если клиентов больше одного?
   Каждое соединение должно иметь уникальную комбинацию source IP + source port + destination IP + destination port, что даёт для одного порта на одном IP ограничение 
   в 2 в степени (32 + 16) соединений, где 2 в 32 степени - это количество ip-адресов (если берем IPv4), а 2 в 16 степени - это количество портов TCP.
   Реальный предел - это файловые дескрипторы. Каждому отдельному соединению сокета дается дескриптор файла, поэтому ограничение на самом деле - это количество файловых дескрипторов в системе.

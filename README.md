...
Выполнив команду ```bash 3>&1 1>&2 2>&3 3>&-``` мы запустим bash в котором дескрипторы stdout и stderr будут поменяны местами, через pipe в stdin будет передаваться уже stderr.
Последняя команда ```3>&-``` закроет файловый дескриптор 3, так как он больше не нужен.  
Работающий пример с выводом stderr команды ls без вывода в консоль и с выводом в файл с использованием pipe будет выглядеть так:
```
vagrant@vagrant:~/devops-netology$ ls -e 3>&1 1>&2 2>&3 | tee -p myfile_2 > /dev/null
vagrant@vagrant:~/devops-netology$ cat myfile_2
ls: invalid option -- 'e'
Try 'ls --help' for more information.
vagrant@vagrant:~/devops-netology$
```
>>>>>>> 2f01361... modified README.md additional homework 3.2
